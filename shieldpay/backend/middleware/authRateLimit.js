...
async function initRedisStore() {
  ...
  try {
    ...
    client.on('error', (err) => {
      logError('[ShieldPay] Redis client error for rate limit store', err)
    })
    ...
    logInfo('[ShieldPay] Auth rate limiter using Redis store')
  } catch (err) {
    logError('[ShieldPay] Redis unavailable for rate limit store, using in-memory fallback', err)
